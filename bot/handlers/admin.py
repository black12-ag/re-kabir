...
def show_stats(update: Update, context: CallbackContext):
    ...
    keyboard = [
        [InlineKeyboardButton(f"👥 View ALL Users (Complete List - {total_users} users)", callback_data="admin_view_all_users")],
        [InlineKeyboardButton(f"👤 View ALL Active Users ({active_users} users)", callback_data="admin_view_active_users")],
        [InlineKeyboardButton(f"📦 View ALL Orders ({total_orders} orders)", callback_data="admin_view_all_orders")],
        [InlineKeyboardButton(f"📦 View ALL Recent Orders ({recent_orders} orders)", callback_data="admin_view_recent_orders")],
        [InlineKeyboardButton("⬅️ Back", callback_data="admin_back")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
def handle_user_id_input(update: Update, context: CallbackContext):
    ...
    if admin_action == 'add_balance':
        ...
        update.message.reply_html(
            ...
            f"Username: {username_display}\n"
            f"🇺🇸 USD Balance: <code>${balance:.2f}</code>\n"
            f"🇪🇹 ETB Balance: <code>ETB {formatted_etb}</code>",
            reply_markup=reply_markup
        )
        return ADMIN_MENU
    ...
    if query.data in ["confirm_remove_balance", "confirm_remove_balance_silent", 
        ...
        if remove_all:
            ...
            if current_balance > 0:
                ...
                query.edit_message_text(
                    f"ℹ️ User {target_user_id} already has $0.00 balance. No action taken.",
                    parse_mode="HTML"
                )
        else:
...
def broadcast_message(update: Update, context: CallbackContext):
    ...
    # Handle regular broadcast (not collection)
        ...
        # Return BROADCASTING state to ensure the callbacks work
        return BROADCASTING
...
def show_referral_settings(update: Update, context: CallbackContext):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Failed to convert referral_threshold to numeric type. Using default: threshold={current_threshold}")
        
        # Get bonus amount using the dedicated function
        current_amount = db.get_referral_bonus_amount()
...
def show_all_users(update: Update, context: CallbackContext, page=1):
    ...
    # Get all users from database without limit
    users = db.get_all_users_list(limit=None)  # Remove limit to show all users
...
def show_active_users(update: Update, context: CallbackContext, page=1):
    ...
    # Get active users from database without limit
    users = db.get_active_users_list(days=7, limit=None)  # Remove limit to show all active users
    ...
    # Create message
    message = (
        f"👤 <b>Active Users</b> (Last 7 days, Total: {total_users})\n\n"
        f"<i>Showing {start_idx + 1}-{end_idx} of {total_users} users</i>\n\n"
        f"{user_list}"
    )
...
def show_all_orders(update: Update, context: CallbackContext, page=1):
    ...
    # Get all orders from database without limit
    orders = db.get_all_orders(limit=None)  # Remove limit to show all orders
    ...
    for i, order in enumerate(page_orders, start_idx + 1):
        ...
        order_list += f"{i}. Order #{order_id} - {username_display} - {service_name} - ${price:.2f} - {status}\n"
...
def show_recent_orders(update: Update, context: CallbackContext, page=1):
    """Show list of recent orders with pagination"""
    query = update.callback_query
    
    # Get recent orders from database without limit
    orders = db.get_recent_orders_list(days=7, limit=None)  # Remove limit to show all recent orders
    ...
    for i, order in enumerate(page_orders, start_idx + 1):
        ...
        order_list += f"{i}. Order #{order_id} - {username_display} - {date_str} - ${price:.2f} - {status}\n"
    
    # Create message
    message = (
        f"📦 <b>Recent Orders</b> (Last 7 days, Total: {total_orders})\n\n"
        f"<i>Showing {start_idx + 1}-{end_idx} of {total_orders} orders</i>\n\n"
        f"{order_list}"
    )
...
def handle_bonus_amount(update: Update, context: CallbackContext):
    """Handle bonus amount input for the new user bonus"""
    try:
        bonus_amount = float(update.message.text.strip())
        if bonus_amount < 0:
            update.message.reply_text("Please enter a non-negative amount.")
            return ENTERING_BONUS_AMOUNT
        
        # Save the bonus amount
        db.set_new_user_bonus_amount(bonus_amount)
        
        # Get current currency
        currency = db.get_new_user_bonus_currency()
        
        keyboard = [
            [InlineKeyboardButton("🔙 Back to Bonus Settings", callback_data="admin_new_user_bonus")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        update.message.reply_html(
            f"✅ <b>New User Bonus Amount Updated</b>\n\n"
            f"The new user bonus amount has been set to {bonus_amount} {currency}.",
            reply_markup=reply_markup
        )
...
def show_welcome_customize_menu(update: Update, context: CallbackContext):
    """Show welcome message customization menu"""
    query = update.callback_query
    query.answer()
    
    # Get current welcome message data
    welcome_data = db.get_custom_message("welcome")
    welcome_text = welcome_data.get("text", "No welcome message set")
    
    # Truncate long text for display
    preview_text = welcome_text[:100] + "..." if len(welcome_text) > 100 else welcome_text
    
    # Create keyboard with options
    keyboard = [
        [InlineKeyboardButton("✏️ Set New Welcome Text", callback_data="admin_set_welcome_direct")],
        [InlineKeyboardButton("➕ Add Media", callback_data="admin_add_welcome_media")],
        [InlineKeyboardButton("🔗 Add Button", callback_data="admin_add_welcome_button")],
        [InlineKeyboardButton("👁️ View Media", callback_data="admin_view_welcome_media")],
        [InlineKeyboardButton("👁️ View Buttons", callback_data="admin_view_welcome_buttons")],
        [InlineKeyboardButton("🔄 Preview", callback_data="admin_preview_welcome")],
        [InlineKeyboardButton("⬅️ Back", callback_data="admin_back")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    # Show current welcome setup
    query.edit_message_text(
        f"✏️ <b>Welcome Message Customization</b>\n\n"
        f"<b>Current welcome text:</b>\n{preview_text}\n\n"
        f"<b>Media items:</b> {db.count_custom_message_media('welcome')}\n"
        f"<b>Buttons:</b> {db.count_custom_message_buttons('welcome')}\n\n"
        f"Select an option to customize the welcome message:",
        reply_markup=reply_markup,
        parse_mode="HTML"
    )
    
    return CUSTOMIZING_WELCOME

def handle_welcome_customize_callback(update: Update, context: CallbackContext):
    """Handle welcome message customization callbacks"""
    query = update.callback_query
    option = query.data
    query.answer()
    
    if option == "admin_set_welcome_direct":
        # Get the current welcome text for reference
        welcome_data = db.get_custom_message("welcome")
        current_text = welcome_data.get("text", "")
        
        # Create a new single-step approach
        query.edit_message_text(
            f"<b>Current welcome text:</b>\n\n{current_text}\n\n"
            f"<b>To set a new welcome text:</b>\n"
            f"1. Copy the current text if you want to use it as a base.\n"
            f"2. Type /setwelcome followed by your new welcome message.\n\n"
            f"Example:\n/setwelcome Welcome to our bot! We're happy to have you here.\n\n"
            f"You can use HTML formatting:\n"
            f"• <b>Bold</b>: &lt;b&gt;text&lt;/b&gt;\n"
            f"• <i>Italic</i>: &lt;i&gt;text&lt;/i&gt;\n"
            f"• <code>Code</code>: &lt;code&gt;text&lt;/code&gt;\n"
            f"• <a href='https://example.com'>Link</a>: &lt;a href='url'&gt;text&lt;/a&gt;",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("⬅️ Back", callback_data="admin_welcome_customize")
            ]])
        )
        return CUSTOMIZING_WELCOME

    # Rest of your existing handler code...
    elif option == "admin_edit_welcome_text":
        # Redirect to the new approach
        return handle_welcome_customize_callback(update, context)
    
    # Keep all other cases the same...
    return CUSTOMIZING_WELCOME

def handle_welcome_media_upload(update: Update, context: CallbackContext):
    """Handle media upload for welcome message"""
    user_id = update.effective_user.id
    ...
    # Clear existing media first - replacement instead of addition
    ...
    if update.message.photo:
        ...
        file_id = update.message.photo[-1].file_id
        media_type = "photo"
        success = db.add_custom_message_media("welcome", media_type, file_id, caption)
        ...
        update.message.reply_text(
            "❌ Unsupported media type. Please send a photo or video.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("⬅️ Back", callback_data="admin_welcome_customize")]
            ])
        )
    ...
    # Clear state
    if "adding_welcome_media" in context.user_data:
        del context.user_data["adding_welcome_media"]
    
    if success:
        # Show confirmation and preview option
        update.message.reply_text(
            f"✅ Welcome message media replaced with new {media_type}!",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔄 Preview Welcome Message", callback_data="admin_preview_welcome")],
                [InlineKeyboardButton("⬅️ Back to Welcome Customization", callback_data="admin_welcome_customize")]
            ])
        )
    else:
        update.message.reply_text(
            f"❌ Failed to set welcome message {media_type}. Please try again.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("⬅️ Back to Welcome Customization", callback_data="admin_welcome_customize")]
            ])
        )
...
def handle_button_text_input(update: Update, context: CallbackContext):
    """Handle input for button text"""
    user_id = update.effective_user.id
    if not is_admin(user_id):
        update.message.reply_text("❌ You don't have permission.")
        return ConversationHandler.END
    ...
    # Store button text in context
    context.user_data["button_text"] = button_text
    
    update.message.reply_text(
        f"🔗 <b>Add Button: {button_text}</b>\n\n"
        f"Now please enter the URL for this button.\n"
        f"This is where users will be directed when they click the button.\n\n"
        f"Type the URL below (must start with http:// or https://):",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("❌ Cancel", callback_data="admin_welcome_customize")]
        ])
    )
    
    return ENTERING_BUTTON_URL

def handle_button_url_input(update: Update, context: CallbackContext):
    ...
    # Check if there's button text in context
    button_text = context.user_data.get("welcome_button_text", "")
    
    if not button_text:
        update.message.reply_text("❌ Button text not found. Please try again.")
        return CUSTOMIZING_WELCOME
    
    # Add button to welcome message
    success = db.add_custom_message_button("welcome", button_text, button_url)
    
    if success:
        update.message.reply_text(
            "✅ Button added successfully to welcome message!",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("⬅️ Back to Welcome Customization", callback_data="admin_welcome_customize")]
            ])
        )
        ...
        update.message.reply_text(
            "❌ Failed to add button. Please try again.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("⬅️ Back to Welcome Customization", callback_data="admin_welcome_customize")]
            ])
        )
    
    # Clear button text from context
    if "welcome_button_text" in context.user_data:
        del context.user_data["welcome_button_text"]
    
    return CUSTOMIZING_WELCOME

def set_welcome_command(update: Update, context: CallbackContext):
    """Handle the /setwelcome command to set a new welcome message directly"""
    user_id = update.effective_user.id
    ...
    if not context.args or len(' '.join(context.args)) == 0:
        update.message.reply_text(
            "⚠️ Please provide the welcome message text after the /setwelcome command.\n\n"
    ...
    # Save to database
    success = db.update_custom_message_text("welcome", welcome_text)
    
    if success:
        # Show confirmation with preview
        update.message.reply_html(
            f"✅ <b>Welcome message set successfully!</b>\n\n"
            f"<b>Preview:</b>\n\n{welcome_text}\n\n"
            f"This message will be shown to users when they start the bot."
        )
    else:
        update.message.reply_text(
...
def set_welcome_media_command(update: Update, context: CallbackContext):
    """Handle the /setwelcomem command to set media for the welcome message (replaces any existing media)"""
    user_id = update.effective_user.id
    ...
    # Mark that we're expecting media upload
    context.user_data["adding_welcome_media"] = True
    
    update.message.reply_text(
        "📸 Please send a photo or video to set for the welcome message.\n\n"
        "This will replace any existing media with the new one you send.\n"
        "The media will appear at the top of your welcome message.\n"
        "You can include a caption with the media if desired.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("❌ Cancel", callback_data="admin_welcome_customize")]
        ])
    )
    
    return WAITING_FOR_WELCOME_MEDIA

def set_referral_bonus_amount_command(update: Update, context: CallbackContext):
    """Command handler for /setreferralbonus to set the referral bonus amount"""
    user_id = update.effective_user.id
    ...
    # Get the bonus amount from arguments
    if not context.args or len(' '.join(context.args)) == 0:
        update.message.reply_text(
            "⚠️ Please provide the bonus amount.\n\n"
            "Example: /setreferralbonus 100"
        )
        return
    ...
    try:
        ...
        if amount <= 0:
            update.message.reply_text("❌ Bonus amount must be greater than 0.")
            return
        
        # Save to database
        db.set_referral_bonus_amount(amount)
        
        # Show confirmation
        update.message.reply_html(
            f"✅ <b>Referral bonus amount updated</b>\n\n"
            f"New referral bonus amount: <b>ETB {amount:.2f}</b>\n\n"
            f"Users will now earn ETB {amount:.2f} for each referral milestone."
        )
        
        logger.info(f"Admin {user_id} set referral bonus amount to {amount}")
        ...
        logger.error(f"Error setting referral bonus amount: {e}")
        update.message.reply_text(
...
admin_conv_handler = ConversationHandler(
    entry_points=[
        ...
        CommandHandler("setwelcomem", set_welcome_media_command)
    ],
    states={
        ADMIN_MENU: [
            CallbackQueryHandler(admin_menu_callback),
            CallbackQueryHandler(broadcast_confirm, pattern=r'^broadcast_confirm$'),
            CallbackQueryHandler(broadcast_confirm, pattern=r'^broadcast_cancel$')
        ],
        CUSTOMIZING_WELCOME: [
            CallbackQueryHandler(handle_welcome_customize_callback, pattern=r'^admin_'),
            MessageHandler(Filters.photo | Filters.video, handle_welcome_media_upload)
        ],
        ENTERING_WELCOME_TEXT: [
            # Removed reference to non-existent function, using direct command instead
            CommandHandler('setwelcome', set_welcome_command)
        ],
        ...
        ENTERING_BUTTON_URL: [
            MessageHandler(Filters.text & ~Filters.command, handle_button_url_input),
            CallbackQueryHandler(handle_welcome_customize_callback, pattern=r'^admin_')
        ],
        WAITING_FOR_WELCOME_MEDIA: [
            MessageHandler(Filters.photo | Filters.video, handle_welcome_media_upload),
            CallbackQueryHandler(handle_welcome_customize_callback, pattern=r'^admin_')
        ],
        ...
        BROADCAST_COLLECTION: [
            CallbackQueryHandler(handle_broadcast_collection_action, pattern=r'^add_collection_'),
            CallbackQueryHandler(handle_broadcast_collection_action, pattern=r'^preview_collection$'),
            CallbackQueryHandler(handle_broadcast_collection_action, pattern=r'^clear_collection$'),
            CallbackQueryHandler(handle_broadcast_collection_action, pattern=r'^send_separate$'),
            CallbackQueryHandler(handle_broadcast_collection_action, pattern=r'^send_group$'),
            CallbackQueryHandler(handle_broadcast_collection_action, pattern=r'^confirm_send_separate$'),
            CallbackQueryHandler(handle_broadcast_collection_action, pattern=r'^confirm_send_group$'),
            CallbackQueryHandler(handle_broadcast_collection_action, pattern=r'^cancel_send$'),
            CallbackQueryHandler(admin_menu_callback, pattern=r'^admin_broadcast$'),
            CallbackQueryHandler(admin_menu_callback, pattern=r'^admin_back$')
        ],
        VIEWING_STATS: [
            CallbackQueryHandler(admin_menu_callback, pattern=r'^admin_back$')
        ...
        ADDING_BALANCE: [
            ...
            CallbackQueryHandler(confirm_add_balance, pattern=r'^confirm_add_balance_silent$'),
        ...
        REMOVING_BALANCE_OPTIONS: [
            ...
            CallbackQueryHandler(confirm_remove_balance, pattern=r'^confirm_remove_all_balance$'),
            CallbackQueryHandler(confirm_remove_balance, pattern=r'^confirm_remove_all_balance_silent$'),
            CallbackQueryHandler(confirm_remove_balance, pattern=r'^confirm_remove_balance$'),
            CallbackQueryHandler(confirm_remove_balance, pattern=r'^confirm_remove_balance_silent$'),
            CallbackQueryHandler(admin_menu_callback, pattern=r'^cancel_remove_balance$'),
            CallbackQueryHandler(admin_menu_callback, pattern=r'^admin_back')
        ],
        ENTERING_USER_ID: [
        ...
        ENTERING_BONUS_AMOUNT: [
            MessageHandler(Filters.text & ~Filters.command, handle_bonus_amount)
        ]
    },
    fallbacks=[
        CommandHandler('cancel', cancel_command),
